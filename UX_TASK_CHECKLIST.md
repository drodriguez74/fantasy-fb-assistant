...
- [ ] `GET /leagues/{id}/comprehensive-analysis` and `GET /leagues/{id}/insights` still return hardcoded demo content (e.g. "Lamar Jackson" as a start/sit recommendation, "Justice Hill"/"Rashod Bateman" as pickup targets) regardless of which league or user is asking — same root cause class as the leagues security fix, just not part of that fix's original scope
- [ ] Blog has genuine duplicate content: 7 saved posts all titled "Week 1 ALL Fantasy Rankings" with no de-duplication guard on save — confirmed live against the current DB
