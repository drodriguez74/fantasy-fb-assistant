...
- [x] **"Game situations"** — resolved 2026-08-29. `historical.py`'s inline third copy was removed previously (frontend already pointed at `POST /game-situations/enhanced-analysis`, backed by `enhanced_game_situation_service.py`). The remaining duplicate, `AdvancedAnalysisService.analyze_game_situations` and its route `POST /advanced-analysis/game-situations`, was confirmed to have zero remaining callers (frontend and backend both grepped) and removed, along with its dedicated helper methods and now-unused imports (`InjuryStatus`, `GameLocation`, `defaultdict`). `enhanced_game_situation_service.py` via `game_situations.py` is now the sole implementation. Verified: `pytest` (73 passed), app imports cleanly, no remaining references anywhere in `backend/` or `frontend/src/`.
...
- [x] **Blog/content**: resolved 2026-09-15. `blog.py` (7 routes, all dead — one route was a literal `"coming soon"` stub) and `app/services/content_service.py` (the abandoned original `ContentGenerationService`, unreachable except through `blog.py`) both removed, along with the router registration. `content.py` + `content_generation_service.py` remains the one shipped, frontend-wired implementation. Verified: `pytest` (102 passed), app imports cleanly, no remaining `frontend/src/` callers.
...
- [x] `backend/app/services/scraper_service.py` — resolved. `_search_player_news`/`_scrape_fantasypros_trending`/`_scrape_espn_trending` mocks were de-fanged to honest no-ops in `f791d8a` (2026-08-23); the file was then trimmed further on 2026-09-15 (alongside the blog/content cleanup above) to just the one method still actually called, `scrape_player_news` — the other two were only reachable through the now-removed dead `blog.py` router, and their own real-scrape siblings (`_scrape_fantasypros_waiver` et al.) were confirmed dead too (fantasypros.com 404, nfl.com permanent redirect).
...
- ~~`backend/app/api/v1/endpoints/blog.py` (all 7 routes; superseded by `content.py`)~~ — removed 2026-09-15, along with `content_service.py`.
...
- ~~`content_service.py` — real code, unreachable except through dead `blog.py`~~ — removed 2026-09-15 alongside `blog.py`.
