...
- [x] Players list renders a "Get AI Insights" button on every one of 4,263 individual player cards with no batching, lazy-loading, or on-demand scoping — a real workflow-friction / performance and cost concern (each is a live API-call trigger), flagged by the fantasy-enthusiast review — `96da20c`: moved AI insights to a new focused `/players/:playerId` detail page (zero extra network cost via router state), removed the per-row button entirely
- [x] Audit remaining "AI-powered" UI copy against what's actually true: content_generation_service.py's `ai_generated` flag was fixed to be honest at the data layer, but front-end copy/labels on pages surfacing this content haven't been checked for consistency with that honesty fix — `7dc8f30`: fixed 4 unconditional "AI-powered" claims (BlogPage, HomePage, LeagueDetailPage, LeaguesPage) that overclaimed vs. what's actually AI-backed, including catching that the Trade Analyzer's own page still said "AI-powered" despite being an explicitly-labeled heuristic
