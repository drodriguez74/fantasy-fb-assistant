...
- Keeper/dynasty support — **out of scope, per the founder directly**: this product is scoped to traditional redraft PPR/Standard leagues; keeper/dynasty support will not be built.
- Push alerts — scoped down to an in-app notification center (real backend-tracked alerts, no browser/device push infrastructure) rather than left unbuilt or over-scoped; see `DEFERRED_FEATURES_CHECKLIST.md` for status.
